# AI Learning Resources

A curated collection of public resources for AI engineering, machine learning, and deep learning.

## Table of Contents

- [Frameworks and Libraries](#frameworks-and-libraries)
- [Learning Resources](#learning-resources)
- [Research Papers and Publications](#research-papers-and-publications)
- [Tools and Platforms](#tools-and-platforms)
- [Datasets and Benchmarks](#datasets-and-benchmarks)
- [Communities and Forums](#communities-and-forums)
- [Best Practices and Guidelines](#best-practices-and-guidelines)
- [Books](#books)

## Frameworks and Libraries

### Deep Learning Frameworks
- **[TensorFlow](https://www.tensorflow.org/)** - End-to-end open source platform for machine learning
- **[PyTorch](https://pytorch.org/)** - Open source machine learning framework based on the Torch library
- **[Keras](https://keras.io/)** - High-level neural networks API, running on top of TensorFlow
- **[JAX](https://github.com/google/jax)** - High-performance numerical computing library with automatic differentiation

### Machine Learning Libraries
- **[scikit-learn](https://scikit-learn.org/)** - Machine learning library for Python
- **[XGBoost](https://xgboost.readthedocs.io/)** - Optimized distributed gradient boosting library
- **[LightGBM](https://lightgbm.readthedocs.io/)** - Fast, distributed, high performance gradient boosting framework
- **[CatBoost](https://catboost.ai/)** - Gradient boosting on decision trees library

### Natural Language Processing
- **[Hugging Face Transformers](https://huggingface.co/transformers/)** - State-of-the-art NLP models
- **[spaCy](https://spacy.io/)** - Industrial-strength Natural Language Processing
- **[NLTK](https://www.nltk.org/)** - Natural Language Toolkit
- **[Gensim](https://radimrehurek.com/gensim/)** - Topic modeling and document similarity

### Computer Vision
- **[OpenCV](https://opencv.org/)** - Open source computer vision and machine learning software library
- **[Detectron2](https://github.com/facebookresearch/detectron2)** - Facebook AI Research's object detection library
- **[YOLO](https://github.com/ultralytics/yolov5)** - Real-time object detection system
- **[MMDetection](https://github.com/open-mmlab/mmdetection)** - Object detection toolbox

## Learning Resources

### Online Courses
- **[Deep Learning Specialization](https://www.coursera.org/specializations/deep-learning)** by Andrew Ng (Coursera)
- **[Machine Learning](https://www.coursera.org/learn/machine-learning)** by Andrew Ng (Coursera)
- **[Fast.ai Practical Deep Learning](https://course.fast.ai/)** - Free deep learning course
- **[MIT Deep Learning](https://deeplearning.mit.edu/)** - Free course from MIT
- **[Stanford CS229: Machine Learning](https://cs229.stanford.edu/)** - Stanford's ML course
- **[Stanford CS231n: Convolutional Neural Networks](http://cs231n.stanford.edu/)** - CNN for Visual Recognition

### Tutorials and Documentation
- **[PyTorch Tutorials](https://pytorch.org/tutorials/)** - Official PyTorch tutorials
- **[TensorFlow Tutorials](https://www.tensorflow.org/tutorials)** - Official TensorFlow tutorials
- **[Kaggle Learn](https://www.kaggle.com/learn)** - Free micro-courses on data science
- **[Machine Learning Mastery](https://machinelearningmastery.com/)** - Practical tutorials
- **[Towards Data Science](https://towardsdatascience.com/)** - Medium publication on AI/ML

### YouTube Channels
- **[3Blue1Brown](https://www.youtube.com/c/3blue1brown)** - Visual explanations of mathematical concepts
- **[Two Minute Papers](https://www.youtube.com/c/KárolyZsolnai)** - Research paper summaries
- **[Lex Fridman](https://www.youtube.com/c/lexfridman)** - AI podcast and interviews
- **[Yannic Kilcher](https://www.youtube.com/c/YannicKilcher)** - Deep learning paper reviews

## Research Papers and Publications

### Essential Papers
- **[Attention Is All You Need](https://arxiv.org/abs/1706.03762)** - Transformer architecture
- **[BERT: Pre-training of Deep Bidirectional Transformers](https://arxiv.org/abs/1810.04805)**
- **[GPT-3: Language Models are Few-Shot Learners](https://arxiv.org/abs/2005.14165)**
- **[ResNet: Deep Residual Learning](https://arxiv.org/abs/1512.03385)**
- **[GAN: Generative Adversarial Networks](https://arxiv.org/abs/1406.2661)**

### Research Archives
- **[arXiv.org](https://arxiv.org/)** - Open-access archive for research papers
- **[Papers with Code](https://paperswithcode.com/)** - Papers with implementation code
- **[Google Scholar](https://scholar.google.com/)** - Academic search engine
- **[Semantic Scholar](https://www.semanticscholar.org/)** - AI-powered research tool

## Tools and Platforms

### Development Environments
- **[Google Colab](https://colab.research.google.com/)** - Free Jupyter notebook environment with GPU
- **[Kaggle Notebooks](https://www.kaggle.com/code)** - Free notebooks with GPU/TPU
- **[Jupyter](https://jupyter.org/)** - Interactive computing notebooks
- **[VS Code](https://code.visualstudio.com/)** - Code editor with AI extensions

### MLOps and Deployment
- **[MLflow](https://mlflow.org/)** - Open source platform for ML lifecycle
- **[Weights & Biases](https://wandb.ai/)** - Experiment tracking and model management
- **[TensorBoard](https://www.tensorflow.org/tensorboard)** - Visualization toolkit
- **[DVC](https://dvc.org/)** - Data version control
- **[Kubeflow](https://www.kubeflow.org/)** - ML toolkit for Kubernetes

### Model Hosting and APIs
- **[Hugging Face Hub](https://huggingface.co/models)** - Model repository and hosting
- **[Replicate](https://replicate.com/)** - Run and deploy ML models
- **[Gradio](https://gradio.app/)** - Build ML web apps quickly
- **[Streamlit](https://streamlit.io/)** - Create data apps easily

## Datasets and Benchmarks

### Dataset Repositories
- **[Kaggle Datasets](https://www.kaggle.com/datasets)** - Wide variety of datasets
- **[UCI Machine Learning Repository](https://archive.ics.uci.edu/ml/)** - Classic ML datasets
- **[Google Dataset Search](https://datasetsearch.research.google.com/)** - Search for datasets
- **[AWS Open Data](https://registry.opendata.aws/)** - Public datasets on AWS
- **[Papers with Code Datasets](https://paperswithcode.com/datasets)** - Research datasets

### Specific Domain Datasets
- **[ImageNet](https://www.image-net.org/)** - Large-scale image database
- **[COCO](https://cocodataset.org/)** - Common Objects in Context dataset
- **[MNIST](http://yann.lecun.com/exdb/mnist/)** - Handwritten digits database
- **[Common Crawl](https://commoncrawl.org/)** - Web crawl data
- **[The Pile](https://pile.eleuther.ai/)** - Large-scale text dataset

## Communities and Forums

### Discussion Forums
- **[Reddit r/MachineLearning](https://www.reddit.com/r/MachineLearning/)** - ML community
- **[Reddit r/deeplearning](https://www.reddit.com/r/deeplearning/)** - Deep learning discussions
- **[Hacker News](https://news.ycombinator.com/)** - Tech news and discussions
- **[Stack Overflow](https://stackoverflow.com/questions/tagged/machine-learning)** - Q&A for developers

### Social Media and Blogs
- **[Twitter AI Community](https://twitter.com/i/lists/1441509971648442371)** - Follow AI researchers
- **[LinkedIn AI Groups](https://www.linkedin.com/)** - Professional networking
- **[Medium AI Publications](https://medium.com/tag/artificial-intelligence)** - AI articles
- **[AI Alignment Forum](https://www.alignmentforum.org/)** - AI safety discussions

### Conferences
- **[NeurIPS](https://neurips.cc/)** - Neural Information Processing Systems
- **[ICML](https://icml.cc/)** - International Conference on Machine Learning
- **[ICLR](https://iclr.cc/)** - International Conference on Learning Representations
- **[CVPR](https://cvpr2023.thecvf.com/)** - Computer Vision and Pattern Recognition

## Best Practices and Guidelines

### Model Development
- **[Google ML Best Practices](https://developers.google.com/machine-learning/guides)** - Guidelines from Google
- **[AWS ML Best Practices](https://aws.amazon.com/machine-learning/ml-best-practices/)** - AWS recommendations
- **[Microsoft Responsible AI](https://www.microsoft.com/en-us/ai/responsible-ai)** - Ethical AI guidelines
- **[ML Engineering Book](http://www.mlebook.com/)** - ML engineering practices

### Code Quality and Testing
- **[pytest](https://docs.pytest.org/)** - Testing framework for Python
- **[black](https://black.readthedocs.io/)** - Python code formatter
- **[flake8](https://flake8.pycqa.org/)** - Python linting
- **[mypy](http://mypy-lang.org/)** - Static type checker for Python

### Data Management
- **[Great Expectations](https://greatexpectations.io/)** - Data validation framework
- **[Pandera](https://pandera.readthedocs.io/)** - Statistical data validation
- **[DVC](https://dvc.org/)** - Data version control

## Books

### Machine Learning
- **"Pattern Recognition and Machine Learning"** by Christopher Bishop
- **"The Elements of Statistical Learning"** by Hastie, Tibshirani, and Friedman
- **"Machine Learning Yearning"** by Andrew Ng (Free online)
- **"Hands-On Machine Learning"** by Aurélien Géron

### Deep Learning
- **"Deep Learning"** by Ian Goodfellow, Yoshua Bengio, and Aaron Courville
- **"Neural Networks and Deep Learning"** by Michael Nielsen (Free online)
- **"Dive into Deep Learning"** by Zhang et al. (Free online)
- **"Deep Learning with Python"** by François Chollet

### AI Ethics and Society
- **"Weapons of Math Destruction"** by Cathy O'Neil
- **"Artificial Intelligence: A Guide for Thinking Humans"** by Melanie Mitchell
- **"The Alignment Problem"** by Brian Christian
- **"Human Compatible"** by Stuart Russell

## Contributing

This is a curated list of resources for AI engineering. Contributions are welcome! Please feel free to submit a pull request with:
- New resources that are high-quality and publicly accessible
- Updates to existing resources
- Corrections or improvements to categorization

## License

This repository is licensed under the MIT License - see the LICENSE file for details.

---

**Note**: This is a living document that will be updated regularly with new resources and improvements.